...
router.post('/licenses', requireAuth, async (req, res) => {
    ...
    try {
        await db.query(`
            ...
            VALUES (?, ?, ?, ?, 'active', ?, ?, ?, ?, ?, '{}', '{}', '[]')
            ON DUPLICATE KEY UPDATE type=VALUES(type), customer_id=VALUES(customer_id), customer_name=VALUES(customer_name),
...
router.patch('/licenses/:key/status', requireAuth, async (req, res) => {
    ...
    try {
        ...
        await addAuditLog('license_status_changed', { license_key: req.params.key, from: rows[0].status, to: req.body.status, by: req.admin.username }, req.admin.username);
...
router.post('/licenses/:key/renew', requireAuth, async (req, res) => {
    try {
        ...
        await db.query("UPDATE licenses SET expires_at = ?, status = 'active' WHERE license_key = ?", [newExpiryStr, req.params.key]);
...
router.patch('/customers/:id', requireAuth, async (req, res) => {
    try {
        ...
        await db.query(`UPDATE customers SET name=COALESCE(?,name), email=COALESCE(?,email),
            ...
            payment_status=COALESCE(?,payment_status), notes=COALESCE(?,notes) WHERE id=?`,
...
router.post('/smtp', requireAuth, requireSuperAdmin, async (req, res) => {
    ...
    if (!host || !user || !pass) return res.status(400).json({ success: false, message: 'Host, Benutzer und Passwort sind Pflichtfelder' });
    try {
        const transporter = createSmtpTransporter({ host, port: port||'587', secure: secure||'false', user, pass });
        await transporter.verify();
        await db.query(`INSERT INTO smtp_config (id, host, port, secure, smtp_user, smtp_pass, smtp_from) VALUES (1,?,?,?,?,?,?)
    ...
    } catch (e) { res.status(400).json({ success: false, message: `SMTP-Verbindungsfehler: ${e.message}` }); }
});
...
router.post('/webhooks', requireAuth, requireSuperAdmin, async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO webhooks (url, secret, events) VALUES (?, ?, ?)', [url, secret||null, JSON.stringify(events||['*'])]);
...
router.post('/impersonate', requireAuth, requireSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        const [custRows] = l.customer_id ? await db.query('SELECT * FROM customers WHERE id = ?', [l.customer_id]) : [[]];
