/**
 * 0009_menu_sort_order.js
 * Ergänzt die sort_order Spalte in der menu Tabelle falls fehlend.
 */
export default async function (conn) {
    // license_key Typ aus licenses-Tabelle auslesen
    const [[licKeyCol]] = await conn.query(`
        SELECT COLUMN_TYPE FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME   = 'licenses'
          AND COLUMN_NAME  = 'license_key'
        LIMIT 1
    `);
    const licKeyType = licKeyCol ? licKeyCol.COLUMN_TYPE : 'VARCHAR(255)';

    // menu Tabelle mit dynamisch angepasstem Typ erstellen
    await conn.query(`
        CREATE TABLE IF NOT EXISTS menu (
            id          INT AUTO_INCREMENT PRIMARY KEY,
            license_key ${licKeyType},
            category    VARCHAR(64),
            name        VARCHAR(255) NOT NULL,
            description TEXT,
            price       DECIMAL(10,2),
            sort_order  INT DEFAULT 0,
            active      TINYINT(1) DEFAULT 1,
            created_at  DATETIME DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (license_key) REFERENCES licenses(license_key) ON DELETE CASCADE
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
    `);

    // sort_order Spalte prüfen und ggf. hinzufügen
    const [[{ n: hasSortOrder }]] = await conn.query(`
        SELECT COUNT(*) AS n FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME   = 'menu'
          AND COLUMN_NAME  = 'sort_order'
    `);
    if (!hasSortOrder) {
        await conn.query(`ALTER TABLE menu ADD COLUMN sort_order INT DEFAULT 0 AFTER price`);
        console.log('  ✅ menu.sort_order hinzugefügt');
    }
}
